...
object WordCountRDDExample {
  ...
  def main(args:Array[String]): Unit = {
    ...
    println("print1-filter-a")
    rdd4.foreach(println)

    val rdd44 = rdd3.filter(a=> a._1.startsWith("b"))
    println("print1-filter-b")
    rdd44.foreach(println)

    //ReduceBy transformation
    val rdd5 = rdd3.reduceByKey(_ + _)
    println("print1-reduceByKey")
    rdd5.foreach(println)

    //Swap word,count and sortByKey transformation
    val rdd6 = rdd5.map(a=>(a._2,a._1)).sortByKey()
    println("print1-sortByKey")
